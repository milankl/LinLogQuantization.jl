...
@testset "LinQuant along dimension" begin
    ...
    Q = LinQuant32Array(A,dim=4)
    @test A ≈ Array{Float32}(Q)

    Q = LinQuant24Array(A,dim=4)
    @test A ≈ Array{Float32}(Q)

    Q = LinQuant16Array(A,dim=4)
    @test A ≈ Array{Float32}(Q)

    Q = LinQuant8Array(A,dim=4)
    @test all(isapprox.(A,Array{Float32}(Q),atol=1e-2))
end

@testset "LogQuant along dimension" begin
    A = rand(Float32,10,20,30,40)
    Q = LogQuant32Array(A,dim=4)
    @test A ≈ Array{Float32}(Q)

    Q = LogQuant24Array(A,dim=4)
    @test A ≈ Array{Float32}(Q)

    Q = LogQuant16Array(A,dim=4)
    @test A ≈ Array{Float32}(Q)

    Q = LogQuant8Array(A,dim=4)
